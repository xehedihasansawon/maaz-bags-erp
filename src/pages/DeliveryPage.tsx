...
function SettlementModal({ order, onClose, onSaved }: SettlementModalProps) {
  ...
  const submit = async () => {
    setSaving(true);
    const isReconciled = form.settlement_reconciled || !hasMismatch;
    const update: Record<string, unknown> = {
      ...
      settlement_reconciled: isReconciled,
    };

    // Auto-verify payment when delivered and cash is settled/reconciled
    if (isDelivered && isReconciled && actualSettlement > 0) {
    ...
    setSaving(false);
    toast.show(
      isDelivered && isReconciled && actualSettlement > 0
        ? 'Settlement saved. Payment auto-verified.'
        : 'Courier settlement saved.',
      'success'
    );
    onSaved();
  };
