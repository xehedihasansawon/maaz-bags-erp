...
import {
  Bag, Inventory, SalesOrder, CourierName, OrderStatus, PaymentStatus,
...
export function SalesPage() {
  ...
  const togglePaymentStatus = async (o: SalesOrder) => {
    const newStatus: PaymentStatus = o.payment_status === 'verified' ? 'unverified' : 'verified';
    const update: Record<string, unknown> = { payment_status: newStatus };
    if (newStatus === 'verified') {
      update.courier_payment_status = 'received';
      update.settlement_reconciled = true;
    }
    const { error } = await supabase
      .from('sales_orders')
      .update(update)
      .eq('id', o.id);
    if (error) toast.show('Failed to update payment status.', 'error');
    else {
      toast.show(`Payment marked as ${newStatus === 'verified' ? 'Verified' : 'Unverified'}.`, 'success');
      load();
    }
  };
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          title={o.payment_status === 'verified' ? 'Click to unverify' : 'Click to verify payment'}
                        >
...
function SalesOrderModal({ open, onClose, editing, bags, variants, pricingMap, onSaved }: SalesOrderModalProps) {
  ...
  const [form, setForm] = useState({
    ...
    courier_tracking_id: '',
    variant_sku: '',
    payment_status: 'unverified' as PaymentStatus,
  });
  ...
  const submit = async () => {
    ...
    const payload = {
      ...
      payment_status: editing ? form.payment_status : 'unverified',
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        {editing && (
          <div className="sm:col-span-2">
            ...
            <p className="mt-1 text-xs text-slate-400">Manually override payment verification status if needed.</p>
          </div>
        )}
      </div>
